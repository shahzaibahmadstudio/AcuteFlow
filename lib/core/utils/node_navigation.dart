...
void navigateFromNode(BuildContext context, DecisionTreeNodeModel node) {
  ...
  if (hasSubCategories) {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => ActivityStateCategoryScreen(parentNode: node),
      ),
    );
  } else {
    Navigator.of(
      context,
    ).push(MaterialPageRoute(builder: (_) => RemedySelectorScreen(node: node)));
  }
}
